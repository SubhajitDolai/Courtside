...
import { Clock, User, Hash, CheckCircle2, XCircle, Copy, Check, Loader } from 'lucide-react'
...
export default function BookingDetailsModal({ bookingId, isOpen, onClose }: BookingDetailsModalProps) {
    ...
    const fetchBookingDetails = async () => {
      ...
      try {
        const { data, error } = await supabase
          .from('bookings')
          .select(`
            id, status, created_at, seat_number, checked_in_at, checked_out_at, booking_date,
            profiles ( first_name, last_name, prn, gender, user_type, phone_number, email, course ),
            sports ( name ),
  ...
  // Reset booking when modal closes
  useEffect(() => {
    if (!isOpen) {
      setBooking(null)
      setCopied(false)
      ...
      <DialogContent className="max-w-lg">
        ...
        {loading ? (
          <div className="flex justify-center items-center py-8">
            ...
            <p className="text-muted-foreground ml-3">Loading booking details...</p>
          </div>
        ) : booking ? (
